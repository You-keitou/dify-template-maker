...
- [[前のタスク]] - {{あれば記載}}
- [[次のタスク]] - {{予定があれば記載}}
- [Claude Code History Viewer](https://github.com/jhlee0409/claude-code-history-viewer) - セッションの可視化
```

7. Git設定とコミット
```bash
# 一時ファイルのクリーンアップ
rm -f /tmp/claude-sessions.txt /tmp/claude-summary.txt
...
正常に完了したら「✅ PR #$ARGUMENTSの作業記録（Claude Codeセッション情報含む）を生成しました」と出力.